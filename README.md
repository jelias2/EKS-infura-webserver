# Infura Web Server
* Welcome to my version of the take home project
...
* * Makefile Commands:
  1. ```make bin``` -> Will build the binary with the artifact sent to /build
  2. ```make binrun``` -> Will build a the binary and run it
  3. ```make clean``` -> Will remove the build directory and perform a go mod tidy
  4. ```make docker``` -> Will build the binary and the dockerized version of the web server
  5. ```make docker-run``` -> Will build the binary, make the docker version, and run it on port 8000 in detached mode
